...
>> print commands.getoutput("python remove-orphans.py -q TestDirs/13_MixedDirRaw testDirs/13_MixedDirRawJPGs
...
if __name__ == '__main__':
    ...
    for filename in raw_files:
        ...
        if re.match(r'(.*)\.(rw|nef|raf)$', filename, re.IGNORECASE):
            ...
            if basename in raw_images_bare_names:
                stderr("'{}' is a duplicate file".format(filename))
            else:
    ...
    if len(raw_images) + len(jpeg_images_bare_names) == 0:
        if verbose:
            stderr(f"No images found. No use checking {args.raw_folder!r} for orphaned RAW images")
        sys.exit(2)
    elif len(raw_images) == 0:
        if verbose:
            print(f"No RAW images found, but {len(jpeg_images_bare_names)} JPEGs. Won't do anything now.")
        sys.exit(0)
    elif len(orphans) == 0:
        if verbose:
            print(f"{len(raw_images)} RAW images found, and {len(jpeg_images_bare_names)} JPEGs but no orphans. "
                  f"Won't do anything now.")
        sys.exit(0)
    ...
    print(f"matched JPEGs:      {matched_jpegs:4d}\n"
          f"unmatched JPEGs:    {len(jpeg_images_bare_names)-matched_jpegs:4d}\n"
          f"orphaned raw moved: {len(orphans):4d}")
